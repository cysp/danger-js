...
if (commander_1.default.args.length === 0) {
    ...
    var findGH = commander_1.default.args.find(function (a) { return a.includes("github"); });
    if (!findGH) {
        console.error("Could find an arg which mentioned GitHub.");
        process.exitCode = 1;
    }
    else {
        var pr = pullRequestParser_1.pullRequestParser(findGH);
        if (!pr) {
            ...
            if (customProcess || validateDangerfileExists_1.default(fileUtils_1.dangerfilePath(commander_1.default))) {
                if (!customProcess) {
